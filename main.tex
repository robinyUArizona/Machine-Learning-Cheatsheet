...
\begin{center}
    ...
    \hline
    \RaggedRight{\includegraphics[scale = .47]{figures/Discriminant_model_figure.PNG}}&
    \RaggedRight{\includegraphics[scale = .47] {figures/Generative_model_figures.PNG}} \\
    \hline
    Learns the decision boundary between classes &
    Learns the input distribution \\
    \hline
    Directly estimate $P(y|x)$ &
    Estimate $P(x|y)$ to find deduce $P(y|x)$ using Baye's rule \\
    \hline
    ...
    Hidden Markov Models, Naive Bayes, Gaussian Mixture Models, Gaussian Discriminant Analysis, LDAm Bayesian Networks
    \end{tabularx}}
\end{center}
...
\vspace{3pt}
\textbf{Batch Gradient Descent} does summing/average of the cost over all the observations. \\
\textbf{Stochastic Gradient Descent} apply the procedure of parameter updating for each observation. \\
...
\subsubsection{Maximum Likelihood Estimation } {\color{teal}\hrule height 0.5pt} \smallskip

MLE is used to find the estimators that \textbf{minimized the likelihood function:} 
...
\href{https://www.youtube.com/watch?v=2dH_qjc9mFg&list=PLKnIA16_RmvYuZauWaPlRTC54KxSNLtNn&index=1}{Deep Learning Tutorial by CampusX} \\


$\rightarrow$ Feeds inputs through different hidden layers and relies on weights and nonlinear functions (activation functions, convolution, or pooling) to reach an output
...
$\bullet$ \textbf{Perceptron} - the foundation of a neural network, and it is a single-layer neural network that multiplies inputs by weights, adds bias, and feeds the result to an activation function \\

\vspace{3pt}

\textbf{Note}: Perceptron is usually used to classify the data into two parts. Therefore, it is also known as a \textbf{Linear Binary Classifier}.
\\
...
% \textbf{Inputs}: Inputs are the set of values for which we need to predict an output value. They can be viewed as features or attributes in a dataset. \\
